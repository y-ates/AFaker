...
void GUI::but_send_listen(GtkWidget* /* unused */, std::vector<Form>& forms){
    g_print("Sending...\r\n");
...
void GUI::choose_mailList(GtkWidget* /* unused */, GtkWidget* parent_window){
    GtkWidget* dialog;
...
void GUI::choose_nameList(GtkWidget* /* unused */, GtkWidget* parent_window){
    GtkWidget* dialog;
    dialog = gtk_file_chooser_dialog_new("Open File",
                         GTK_WINDOW(parent_window),
                         GTK_FILE_CHOOSER_ACTION_OPEN,
...
GtkWidget* GUI::create_checkbox(const char* label, GtkWidget* txtField){
    ...
    widgetContainer box_field;
    box_field.first = cbox;
    box_field.second = txtField;
    
    g_signal_connect(GTK_WIDGET(cbox), "toggled",
                     G_CALLBACK(GUI::chkbox_listen), &box_field);
...
void GUI::chkbox_listen(widgetContainer context){
    GtkWidget* cbox = context.first;
    GtkWidget* txtField = context.second;
